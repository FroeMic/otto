  ...
  return (
    ...
    >
      ...
      <LandingSection className="pb-18 pt-18 md:pb-24 md:pt-24" id="for-whom">
        ...
        <div className="mx-auto mt-10 grid max-w-6xl gap-6 md:grid-cols-3">
          {landingAudienceCards.map((card) => (
            <article
              className="rounded-lg border border-border/70 bg-background px-6 py-6 shadow-[0_10px_24px_rgba(15,23,42,0.04)]"
              key={card.title}
            >
      ...
      <LandingSection className="pb-18 md:pb-24">
        ...
        <div className="mx-auto mt-10 grid max-w-6xl gap-6 md:grid-cols-3">
          {landingPainCards.map((card) => (
            <article
              className="rounded-lg border border-border/70 bg-[#fdfcf8] px-6 py-6 shadow-[0_10px_24px_rgba(15,23,42,0.04)]"
              key={card.title}
            >
              <h3 className="text-2xl font-semibold tracking-tight text-foreground">
                {card.title}
      ...
      <LandingSection className="pb-18 md:pb-24" id="how-it-works">
        <div className="mx-auto grid max-w-6xl gap-12 lg:grid-cols-[1.08fr_0.92fr] lg:items-start">
          ...
          <div className="grid gap-4">
            <div className="rounded-lg border border-border/70 bg-[#f7f4ef] p-6 shadow-[0_18px_44px_rgba(15,23,42,0.06)]">
              <div className="flex items-center justify-between text-xs text-muted-foreground">
                <span>Chat with Otto</span>
                <span>What happens next</span>
              </div>

              <div className="mt-5 rounded-md border border-border/65 bg-background px-4 py-4">
              ...
              <div className="mt-5 grid gap-4 md:grid-cols-[1.05fr_0.95fr]">
                <div className="rounded-md border border-border/65 bg-background px-4 py-4">
            ...
            <div className="grid gap-4 sm:grid-cols-3">
              {landingOperatingPillars.map((pillar) => (
                <article
                  className="rounded-md border border-border/70 bg-background px-5 py-5 shadow-[0_10px_24px_rgba(15,23,42,0.04)]"
                  key={pillar.title}
                >
      ...
      <LandingSection className="pb-20 md:pb-28" id="proof">
        <div className="mx-auto max-w-6xl rounded-lg border border-border/70 bg-background px-8 py-8 shadow-[0_12px_32px_rgba(15,23,42,0.05)]">
          <div className="grid gap-10 lg:grid-cols-[0.9fr_1.1fr] lg:items-center">
            ...
            <div className="grid gap-4 sm:grid-cols-3">
              {landingOperatingPillars.map((pillar) => (
                <article
                  className="rounded-md border border-border/70 bg-[#f7f4ef] px-5 py-5"
                  key={pillar.title}
                >
                  <h3 className="text-xl font-semibold tracking-tight text-foreground">
                    {pillar.title}
