...
export class GandiApiError extends Error {
  ...
  constructor(input: {
    ...
    super(
      `Gandi API request failed with status ${input.status}: ${input.message}`,
    )
...
export async function getGandiDomainRegistrationMetadata(domain: string) {
  ...
  const [availabilityResponse, tldResponse] = await Promise.all([
    ...
    fetchGandiJson<GandiTldResponse>(
      `/domain/tlds/${encodeURIComponent(inferTld(normalizedDomain) ?? normalizedDomain)}`,
    ),
  ])

  const product = findAvailabilityProduct(
...
export async function getGandiDomainDetails(domain: string) {
  ...
  return {
    ...
    domain:
      typeof response.fqdn === "string" ? response.fqdn : normalizedDomain,
...
function normalizeDnsRecord(
  record: GandiLiveDnsRecordResponse,
): GandiDnsRecord {
  return {
...
function getGandiApiBaseUrl() {
  const baseUrl =
    process.env.GANDI_BASE_URL?.trim() || DEFAULT_GANDI_API_BASE_URL
