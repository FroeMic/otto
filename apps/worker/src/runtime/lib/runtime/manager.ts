...
export class RuntimeManager {
  ...
  async checkGatewayHealthWithResult(connection: SshConnection) {
    try {
      ...
      for (let attempt = 1; ; attempt += 1) {
        ...
        await sleep(Math.min(getGatewayHealthPollDelayMs(attempt), remainingMs))
      }
  ...
  async forwardWorkspaceChatIngressRequest(
    ...
    console.info(
      "[workspace-chat] runtime manager posting tenant ingress request",
      {
  ...
  private async getGatewayDiagnostics(connection: SshConnection) {
    ...
    return (result.stdout || result.stderr || "No diagnostics available").trim()
  }
...
function parseToolInvokePayload(value: string) {
  ...
  throw new Error("Tenant runtime tool response did not include a JSON payload")
}
...
function parseWorkspaceChatHttpIngressPayload(
  ...
  if (payload.ok !== true || payload.accepted !== true) {
    throw new Error(
      explicitError ||
        "Tenant runtime workspace chat ingress did not acknowledge the event",
    )
  }
