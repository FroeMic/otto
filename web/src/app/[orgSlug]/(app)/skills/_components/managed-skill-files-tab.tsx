...
type WorkspaceSkillDirectoryFileSnapshot = {
...
type WorkspaceSkillDirectorySnapshot = {
...
type ExplorerTreeNode =
  ...
  | {
      file: WorkspaceSkillDirectoryFileSnapshot;
      kind: "file";
      name: string;
      path: string;
    };
...
export function ManagedSkillFilesTab({ orgSlug, skillKey }: Props) {
    ...
    async function loadFiles() {
      ...
      try {
        const response = await fetch(
          `/api/workspace/${encodeURIComponent(orgSlug)}/skills/${encodeURIComponent(skillKey)}/files`,
          {
            cache: "no-store",
          },
        );
        const payload = (await response.json()) as
          | {
              code?: string;
              message?: string;
              snapshot?: WorkspaceSkillDirectorySnapshot;
            }
          | undefined;

        if (!response.ok || !payload?.snapshot) {
          throw new Error(payload?.message ?? "Failed to load skill files.");
        }
  ...
  async function handleRefresh() {
    ...
    try {
      ...
      if (!response.ok || !payload?.snapshot) {
        throw new Error(payload?.message ?? "Failed to refresh skill files.");
      }
      ...
      <div className="rounded-2xl border border-border bg-card shadow-sm">
        <div className="flex flex-wrap items-center justify-between gap-3 border-b border-border px-4 py-3">
          <div className="flex min-w-0 flex-col gap-1">
            ...
            <div className="truncate font-mono text-sm text-foreground">
              {snapshot?.rootPath ?? `skills/${skillKey}`}
            </div>
          </div>
        ...
        {!snapshot?.rootExists ? (
          <div className="flex h-[38rem] items-center justify-center px-6 text-center text-sm text-muted-foreground">
            This skill package exists in the workspace, but the projected
            runtime directory has not appeared yet.
          </div>
        ) : snapshot.files.length === 0 ? (
          <div className="flex h-[38rem] items-center justify-center px-6 text-center text-sm text-muted-foreground">
            This skill directory is currently empty.
          </div>
        ) : (
          <>
...
function EditorPane(input: {
  selectedFile: WorkspaceSkillDirectoryFileSnapshot | null;
  theme: "vs" | "vs-dark";
}) {
...
function buildExplorerTree(files: WorkspaceSkillDirectoryFileSnapshot[]) {
