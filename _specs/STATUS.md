...
- The next developer-workflow integration slice is now tracked in `_specs/TODO_39_github_integration.md`:
  ...
  - first implementation slices on `plan/github-integration` now register the GitHub integration catalog, complete repository/branch/pull-request/issue command groups, the `github_app_installation` framework auth binding, GitHub App JWT/installation-token helpers, GitHub installation/repository database state, signed GitHub App install start/callback routes, repository sync, and a basic workspace settings page that can connect/reconnect/disconnect and show account plus repository count; the next slice should add executable read commands and a repository selection management UI
