...
- The OpenClaw `2026.4.21` runtime-base upgrade now lives in `TODO_27_openclaw_runtime_upgrade.md`:
  ...
  - the repo defaults now point at OpenClaw `2026.4.21`, custom image revision `2026.4.21.3`, and runtime plugin package versions `2026.4.21.3`
  ...
  - follow-on tenant testing also found workspace voice-note transcription failing with `Audio transcription failed (HTTP 404)` before the model turn; `otto-ai-provider` now normalizes raw OpenAI proxy media base URLs to `/api/internal/runtime/ai/openai/v1`, sanitizes literal missing audio model values such as `undefined`, and the control-plane audio proxy repairs invalid multipart model fields while logging safe request diagnostics and upstream error bodies; this fix requires publishing the new `ghcr.io/froemic/otto-openclaw:2026.4.21.3` custom runtime image
  ...
  - target custom image remains `ghcr.io/froemic/otto-openclaw:2026.4.21.3`; rollback remains `ghcr.io/froemic/otto-openclaw:2026.4.15.3`
