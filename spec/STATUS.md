...
- The next managed-integrations refactor slice is now in progress on `main`:
  ...
  - the `cycle.*` slice is now started with `cycle.list`, `cycle.get`, `cycle.create`, and `cycle.update`
  - the next recommended Linear commands are `cycle.archive` and `cycle.list_issues`
