...
describe("runtime integration registry", () => {
  ...
  it("builds a sorted manifest for supported keys only", () => {
    ...
    assert.deepEqual(
      ...
      ["customer_need", "customer", "customer_tier", "customer_status", "initiative", "cycle", "workspace", "user", "document", "label", "project_milestone", "project_status", "issue", "project", "comment"],
    );
    assert.equal(
    ...
    assert.equal(
      manifest[0]?.commandGroups.find((group) => group.groupKey === "initiative")
        ?.commandCount,
      7,
    );
    assert.equal(
