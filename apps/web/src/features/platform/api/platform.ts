  ...
  return queryOptions({
    queryFn: async (): Promise<PlatformBootstrap> => {
      ...
      return fetchApiResponse(response, (data) =>
        platformBootstrapSchema.parse(data),
      )
    },
...
export async function addCurrentUserAsPlatformOrganizationAdmin(
  ...
  const response = await apiClient.api.platform.organizations[":orgSlug"][
    "admin-membership"
  ].$post({
    param: {
      orgSlug,
...
export function platformOrganizationDetailQueryOptions(orgSlug: string) {
  return queryOptions({
    queryFn: async (): Promise<PlatformOrganizationDetailResponse> => {
      const response = await apiClient.api.platform.organizations[
        ":orgSlug"
      ].$get({
        param: {
          orgSlug,
...
export function platformUsageQueryOptions(input: {
  ...
  return queryOptions({
    queryFn: async (): Promise<PlatformUsage> => {
      ...
      return fetchApiResponse(response, (data) =>
        platformUsageSchema.parse(data),
      )
    },
...
export async function fetchPlatformJobStatus(input: {
  ...
  const response = await apiClient.api.platform.organizations[":orgSlug"].jobs[
    ":jobId"
  ].status.$get({
    param: {
      jobId: input.jobId,
