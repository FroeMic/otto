...
- The monorepo now also contains the first Otto-owned OpenClaw plugin layer:
  ...
  - the plugin implementation is now aligned with the released OpenClaw `2026.3.13` native plugin shape (plain exported plugin object + `configSchema`) instead of the newer helper-based API
