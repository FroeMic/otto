    ...
    >
      ...
      <LandingSection className="pb-18 pt-18 md:pb-24 md:pt-24" id="for-whom">
        ...
        <div className="mt-10 grid gap-6 md:grid-cols-3">
          {landingAudienceCards.map((card) => (
            <article
              className="rounded-[2rem] border border-border/70 bg-background px-6 py-6 shadow-[0_10px_24px_rgba(15,23,42,0.04)]"
              key={card.title}
            >
      ...
      <LandingSection className="pb-18 md:pb-24">
        ...
        <div className="mt-10 grid gap-6 md:grid-cols-3">
          {landingPainCards.map((card) => (
            <article
              className="rounded-[2rem] border border-border/70 bg-[#fdfcf8] px-6 py-6 shadow-[0_10px_24px_rgba(15,23,42,0.04)]"
              key={card.title}
            >
              <h3 className="text-2xl font-semibold tracking-tight text-foreground">
                {card.title}
      ...
      <LandingSection className="pb-18 md:pb-24" id="how-it-works">
        <div className="grid gap-12 lg:grid-cols-[1.08fr_0.92fr] lg:items-start">
          ...
          <div className="rounded-[2.25rem] border border-border/70 bg-background p-6 shadow-[0_18px_44px_rgba(15,23,42,0.06)]">
            <div className="rounded-[1.7rem] border border-border/65 bg-[#f7f4ef] p-5">
              <div className="flex items-center justify-between text-xs text-muted-foreground">
                <span>Otto operating brief</span>
                <span>Qualify the next step</span>
              </div>

              <div className="mt-5 rounded-[1.25rem] border border-border/65 bg-background px-4 py-4">
              ...
              <div className="mt-4 grid gap-3">
                <div className="rounded-[1.2rem] border border-border/65 bg-background px-4 py-4">
                ...
                <div className="rounded-[1.2rem] border border-border/65 bg-background px-4 py-4">
                  ...
                  <div className="mt-3 grid gap-3 sm:grid-cols-3">
                    {landingOperatingPillars.map((pillar) => (
                      <div
                        className="rounded-[1rem] border border-border/60 bg-[#fdfcf8] px-3 py-3"
                        key={pillar.title}
                      >
      ...
      <LandingSection className="pb-20 md:pb-28" id="proof">
        <div className="rounded-[2.25rem] border border-border/70 bg-background px-8 py-8 shadow-[0_12px_32px_rgba(15,23,42,0.05)]">
          <div className="grid gap-10 lg:grid-cols-[0.9fr_1.1fr] lg:items-center">
            ...
            <div className="grid gap-4 sm:grid-cols-3">
              {landingOperatingPillars.map((pillar) => (
                <article
                  className="rounded-[1.5rem] border border-border/70 bg-[#f7f4ef] px-5 py-5"
                  key={pillar.title}
                >
