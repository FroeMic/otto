...
class FluidSim {
  ...
  getFbo(name: string) {
    const fbo = this.fbos[name];

    if (!fbo) {
      throw new Error(`Missing fluid render target: ${name}`);
    }
  ...
  private _renderer = () => {
    if (!this.gl.renderer) {
      throw new Error("Missing WebGL renderer");
    }

    return this.gl.renderer;
  };
  ...
  private _createPasses() {
    const { cellScale, fboSize, opts } = this;
    ...
    const advUniforms: Uniforms = {
      ...
      velocity: { value: this.getFbo("vel_0").texture },
      dt: { value: opts.dt },
      isBFECC: { value: true },
    };
    ...
    const viscPass = new ShaderPass(
      ...
      {
        boundarySpace: { value: cellScale },
        velocity: { value: this.getFbo("vel_1").texture },
        velocity_new: { value: this.getFbo("vel_v0").texture },
    ...
    this.viscousPass = {
      pass: viscPass,
      output0: this.getFbo("vel_v0"),
      output1: this.getFbo("vel_v1"),
    };
    ...
    this.divergencePass = new ShaderPass(
      ...
      {
        boundarySpace: { value: cellScale },
        velocity: { value: this.getFbo("vel_v0").texture },
        px: { value: cellScale },
        dt: { value: opts.dt },
      },
      this.getFbo("div"),
    ...
    const poisPass = new ShaderPass(
      ...
      {
        boundarySpace: { value: cellScale },
        pressure: { value: this.getFbo("p0").texture },
        divergence: { value: this.getFbo("div").texture },
        px: { value: cellScale },
      },
      this.getFbo("p1"),
    );
    this.poissonPass = {
      pass: poisPass,
      output0: this.getFbo("p0"),
      output1: this.getFbo("p1"),
    };
  ...
  update(time: number) {
    ...
    if (opts.isViscous) {
      ...
      for (let i = 0; i < opts.iterations_viscous; i++) {
        ...
        u.velocity_new.value = fboIn.texture;
        pass.render(fboOut);
      }
      velFBO = fboOut;
    }
...

    {
      ...
      for (let i = 0; i < opts.iterations_poisson; i++) {
        ...
        pass.uniforms.pressure.value = pIn.texture;
        pass.render(pOut);
      }
...
export function PixelLiquidBg({
  ...
  useEffect(() => {
    ...
    const outputUniforms: Uniforms = {
      velocity: { value: sim.getFbo("vel_0").texture },
