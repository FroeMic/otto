...
- The next managed-integrations refactor slice is now in progress on `main`:
  ...
  - the `cycle.*` slice is now started with `cycle.list`, `cycle.get`, and `cycle.create`
  - the next recommended Linear commands are `cycle.update`, `cycle.archive`, and `cycle.list_issues`
