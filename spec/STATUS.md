...
- The next managed-integrations refactor slice is now in progress on `main`:
  ...
  - the `cycle.*` slice is now started with `cycle.list`, `cycle.get`, `cycle.create`, `cycle.update`, and `cycle.archive`
  - the next recommended Linear command is `cycle.list_issues`
