...
export interface IntegrationStickySaveBarProps {
  description: string
  discardLabel?: string
  hasChanges: boolean
  isPending: boolean
...
export function IntegrationStickySaveBar({
  description,
  ...
  title,
}: IntegrationStickySaveBarProps) {
  if (!hasChanges || isPending) {
    return null
  }

  return (
    <div className="pointer-events-none fixed bottom-10 left-6 right-6 z-30 sm:left-[max(1.5rem,calc(50%-24rem))] sm:right-auto sm:w-[min(100%-3rem,48rem)]">
