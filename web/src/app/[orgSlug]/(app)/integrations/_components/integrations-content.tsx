...
function categorize(surfaces: SurfaceEntry[]): CategorySection[] {
  ...
  const integrations = surfaces.filter((s) => s.uiGroup === "integrations");
  ...
  const tools = surfaces.filter((s) => s.uiGroup === "tools");
  if (tools.length > 0) {
    sections.push({ key: "tools", label: "Tools", surfaces: tools });
  }
