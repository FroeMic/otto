...
- The next `TODO_17_managed_integrations_architecture.md` increments are now implemented on `main`:
  ...
  - runtime Linear now exposes grouped commands such as `workspace.get_viewer`, `workspace.list_teams`, `workspace.list_users`, `workspace.list_workflow_states`, `issue.search`, `issue.get`, `issue.list`, `issue.create`, `issue.update`, `issue.archive`, `issue.batch_update`, `issue.list_comments`, `issue.list_attachments`, `issue.list_documents`, `issue.list_relations`, `issue.add_label`, and `issue.remove_label`
...
- The next managed-integrations refactor slice is now in progress on `main`:
  ...
  - the next recommended Linear slice is to finish the workspace foundation with `workspace.get_organization` and `workspace.list_project_statuses`, then move into the adjacent `comment.*` and `project.*` slices
