...
const SEARCH_LINEAR_ISSUES_QUERY = `
  query OttoSearchLinearIssues($query: String!, $limit: Int!) {
    issues(
      first: $limit
      filter: {
        or: [
          ...
          { project: { name: { containsIgnoreCase: $query } } }
          { team: { name: { containsIgnoreCase: $query } } }
          { assignee: { name: { containsIgnoreCase: $query } } }
        ]
      }
    ) {
      nodes {
...
export async function searchLinearIssues(input: SearchLinearIssuesInput) {
  ...
  const items = (payload.data?.issues?.nodes ?? []).map((issue) => ({
