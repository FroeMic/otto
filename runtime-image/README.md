...
OpenClaw `2026.4.12` resolves bundled plugins from `/app/dist/extensions` in the
...
docker build \
  ...
  --build-arg OPENCLAW_BASE_IMAGE=ghcr.io/openclaw/openclaw:2026.4.12 \
  -t ghcr.io/froemic/otto-openclaw:2026.4.12.1 .
```
...
```bash
IMAGE_REVISION=1 \
OPENCLAW_BASE_IMAGE=ghcr.io/openclaw/openclaw:2026.4.12 \
...
```bash
RUNTIME_OPENCLAW_IMAGE=ghcr.io/froemic/otto-openclaw:2026.4.12.1
```
